...
from tools_utils.plot import plot_matches, warp_img_and_blend, warp_img, epipolar_error
...
def parse_args():
    ...
    parser.add_argument(
        '--baseline_config_path', type=str, default=None)
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if method != 'None':
        ...
        matcher = PL_LoFTR(config, pretrained_ckpt=args.ckpt_path, test_mode=True, baseline_config=baseline_model_config).matcher
        matcher.eval().cuda()
    else:
        matcher = None
    ...
    eval_mode = 'gt_homo' # ['gt_homo', 'gt_match', 'pose', 'competition_submission]
    for id, data in enumerate(tqdm(dataloader)):
        ...
        elif 'gt_2D_matches' in data and data["gt_2D_matches"].shape[-1] == 2:
            eval_mode = 'competition_submission'
            ransac_mode = 'affine'
        ...
        if matcher is not None:
            if eval_mode in ['gt_match', 'competition_submission']:
            ...
            elif 'affine_first_and_then_elastix' in estimator:
                from tools.tools_utils.non_rigid_refinement import SimpleElastixWarper
                inverse = False
                if inverse:
                    logger.error("for plotting only, img0, img1 is inverse acutally!")
                    mkpts0_, mkpts1_ = mkpts1, mkpts0
                    img0_, img1_ = img1, img0
                else:
                    mkpts0_, mkpts1_ = mkpts1, mkpts0
                    img0_, img1_ = img0, img1

                # Estimate affine:
                H_est, inlier_mask = estimate_homo(mkpts0_, mkpts1_, thresh=args.elastix_ransac_thr, mode=ransac_mode)
                if inlier_mask is not None:
                    mkpts0_, mkpts1_, mconf_ = map(lambda x: x[inlier_mask[:, 0].astype(bool)], [mkpts0_, mkpts1_, mconf])
                else:
                    mkpts0_, mkpts1_, mconf_ = mkpts0_, mkpts1_, mconf
                    H_est = np.eye(3)

                # Warp imgs and pts:
                eval_points_rigid_warpped = warp_points(eval_coord, H_est, inverse=False)
                mkpts0_ = warp_points(mkpts0_, H_est, inverse=False)
                img0_ = warp_img(img0_, img1_, H_est, fill_white=True)

                try:
                    sitkwarper = SimpleElastixWarper()
                    eval_points_warpped = sitkwarper.calc(array_rgb2gray(img1_.astype(np.float32)), array_rgb2gray(img0_.astype(np.float32)), moving_xy=mkpts1_, fixed_xy=mkpts0_, src_pts=eval_points_rigid_warpped, warped_img_save_path=Path(vis_output_path)/'aligned'/f"{pair_name}_{args.method}.png" if args.plot_align else None, moving_img_src=img1_, fixed_img_src=img0_)

                except:
                    logger.error(f"Elastix refine failed!")
                    eval_points_warpped = eval_points_rigid_warpped
            elif estimator == 'pose':
        ...
        if eval_mode != 'competition_submission':
            ...
            assert len(eval_coord) == len(eval_points_warpped)
            thr = None
            result_dict['@-@'.join([data['pair_names'][0][0].split('/', 1)[1], data['pair_names'][1][0].split('/', 1)[1]])] = eval_points_warpped
