...
from src.utils.dataset import read_megadepth_depth_gray, read_megadepth_gray, read_megadepth_depth


class MegaDepthDatasetCrossModal(Dataset):
    def __init__(self,
                 root_dir,
                 npz_path,
                 mode='train',
        ...
        self.probability = probability # [rgb2rgb, rgb2depth, depth2rgb, depth depth]
        assert self.probability[-1] <= 1.0
    ...
    def __getitem__(self, idx):
        ...
        depth_path0 = osp.join(self.root_dir, self.dataset_name, self.scene_info['depth_paths'][idx0])
        depth_path1 = osp.join(self.root_dir, self.dataset_name, self.scene_info['depth_paths'][idx1])
        ...
        if self.mode == 'train':
            ...
            if random_control < self.probability[0]:
            ...
            elif random_control < self.probability[1]:
                image0, mask0, scale0, origin_img_size0 = read_megadepth_depth_gray(
                    depth_path0, self.img_resize, self.df, self.img_padding, None, read_gray=self.read_gray, normalize_img=self.normalize_img, resize_by_stretch=self.resize_by_stretch)
                image1, mask1, scale1, origin_img_size1 = read_megadepth_gray(
                    img_name1, self.img_resize, self.df, self.img_padding, None, read_gray=self.read_gray, normalize_img=self.normalize_img, resize_by_stretch=self.resize_by_stretch)
            elif random_control < self.probability[2]:
                image0, mask0, scale0, origin_img_size0 = read_megadepth_gray(
                    img_name0, self.img_resize, self.df, self.img_padding, None, read_gray=self.read_gray, normalize_img=self.normalize_img, resize_by_stretch=self.resize_by_stretch)
                image1, mask1, scale1, origin_img_size1 = read_megadepth_depth_gray(
                    depth_path1, self.img_resize, self.df, self.img_padding, None, read_gray=self.read_gray, normalize_img=self.normalize_img, resize_by_stretch=self.resize_by_stretch)
            else:
                image0, mask0, scale0, origin_img_size0 = read_megadepth_depth_gray(
                    depth_path0, self.img_resize, self.df, self.img_padding, None, read_gray=self.read_gray, normalize_img=self.normalize_img, resize_by_stretch=self.resize_by_stretch)
                image1, mask1, scale1, origin_img_size1 = read_megadepth_depth_gray(
