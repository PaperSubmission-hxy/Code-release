...
from src.utils.dataset import read_megadepth_gray, read_megadepth_gray_sample_homowarp
from src.utils.homography_utils import compute_homography
from src.loftr.utils.geometry import homo_warp_kpts


class CommonDatasetHomoWarp(Dataset):
    def __init__(self,
                 root_dir,
                 npz_path,
                 mode='train',
        ...
        self.homo_warp_use_mask = homo_warp_use_mask
        ...
        self.gt_matches = self.scene_info['gt_matches'] if 'gt_matches' in self.scene_info else None # sparse matches produced by teacher models
    ...
    def __getitem__(self, idx):
        ...
        img_name1 = osp.join(self.root_dir, self.dataset_name, self.scene_info['image_paths'][idx1]) # Often transformed image based on img0, e.g., depth estimation or Diffusion
        ...
        image1, mask1, scale1, origin_img_size1, norm_pixel_mat, homo_sampled_normed = read_megadepth_gray_sample_homowarp(
        ...
        # mask for depth map  
        if 'mask_paths' in self.scene_info and self.homo_warp_use_mask:
            mask_name0 = osp.join(self.root_dir, self.dataset_name, self.scene_info['mask_paths'][idx0])
            mask_name1 = osp.join(self.root_dir, self.dataset_name, self.scene_info['mask_paths'][idx1])
            # To mask sky or other undesired things:
            homo_mask0, _, _, _ = read_megadepth_gray(
                                    mask_name0, self.img_resize, self.df, self.img_padding, None, read_gray=True, resize_by_stretch=self.resize_by_stretch)
            homo_mask1, _, _, _ = read_megadepth_gray(
                                    mask_name1, self.img_resize, self.df, self.img_padding, None, read_gray=True, resize_by_stretch=self.resize_by_stretch)
        else:
        ...
        if self.fp16:
            data = {
                ...
                'norm_pixel_mat': norm_pixel_mat,
                'homo_sample_normed': homo_sampled_normed,
        ...
        if self.gt_matches is not None:
            ...
            if isinstance(gt_matches_, str):
                try:
                    ...
                    logger.error(f"GT match load error!")
                    gt_matches_ = np.empty((0, 4))
            gt_matches_ = torch.from_numpy(gt_matches_).to(torch.float) # N * 4: mkpts0, mkpts1
            # Warp mkpts1 by sampled homo:
            valid_mask, warpped_mkpts1 = homo_warp_kpts(gt_matches_[:, 2:][None], norm_pixel_mat[None], homo_sampled_normed[None], original_size0=origin_img_size1[None], original_size1=origin_img_size1[None])
            gt_matches_ = torch.cat([gt_matches_[:, :2], warpped_mkpts1[0]], dim=-1)[valid_mask[0]]
