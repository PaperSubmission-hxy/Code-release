...
from src.losses.robust_loss import RobustLosses
...
from third_party.ROMA.roma.roma_self_train_adapt_model import ROMA_SELF_TRAIN_Model
...
class PL_LoFTR(pl.LightningModule):
    def __init__(self, config, pretrained_ckpt=None, profiler=None, dump_dir=None, test_mode=False, baseline_config=None):
        ...
        # Matcher: LoFTR
        if config.METHOD == 'loftr':
            self.matcher = LoFTR(config=_config['loftr'], profiler=self.profiler)
            self.loss = LoFTRLoss(_config)
        elif config.METHOD == "ROMA_SELF_TRAIN":
            self.matcher = ROMA_SELF_TRAIN_Model(config=_config['roma'], test_mode=test_mode)
            self.loss = RobustLosses(
                ignore_empty_in_sparse_match_spv=_config['loftr']['roma_loss']['ignore_empty_in_sparse_match_spv'],
                ce_weight=0.01, 
        ...
        # Pretrained weights
        if pretrained_ckpt:
            if config.METHOD in ['loftr', "ROMA_SELF_TRAIN"]:
