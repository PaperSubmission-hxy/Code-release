...
from .MultiviewMatcher.MultiviewMatcher import MultiviewMatcher
from ..utils.data_io import dict_to_cuda
from ..dataset.construct_matching_data import MatchingMultiviewData
...
def matchWorker(colmap_dataset, matcher, subset_track_idxs=None, visualize=False, visualize_dir=None, pba: ActorHandle = None, dataset_cfgs=None, verbose=True):
    ...
    dataloader = DataLoader(multiview_matching_dataset, num_workers=2, pin_memory=True)
...
def matchWorker_ray_wrapper(*args, **kwargs):
    try:
        ...
        logger.error(f"Error captured in parallel refinement matching:{e}")