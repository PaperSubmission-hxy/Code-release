...
def make_matching_figure(
    ...
    if draw_detection and kpts0 is not None and kpts1 is not None:
        # color = 'g'
        color_det = 'r'
        axes[0].scatter(kpts0[:, 0], kpts0[:, 1], c=color_det, s=1 * plot_size_factor)
        axes[1].scatter(kpts1[:, 0], kpts1[:, 1], c=color_det, s=1 * plot_size_factor)
    ...
    if draw_match_type is 'corres':
        ...
        axes[0].scatter(mkpts0[:, 0], mkpts0[:, 1], c=color[:, :3], s=0.5* plot_size_factor)
        axes[1].scatter(mkpts1[:, 0], mkpts1[:, 1], c=color[:, :3], s=0.5* plot_size_factor)
        ...
        axes[0].scatter(mkpts0[:, 0], mkpts0[:, 1], c=kpts_color, s=1)
        axes[1].scatter(mkpts1[:, 0], mkpts1[:, 1], c=kpts_color, s=1)
...
def matching_coord2color(kpts, img_h, img_w, r_normalize_factor=0.4, white_center=True):
    """
    ...
    """
    kpts = np.copy(kpts)
    r_max = np.sqrt((img_h / 2) ** 2 + (img_w / 2) ** 2)
    kpts[:, 0] = kpts[:, 0] - img_w / 2  # x
    kpts[:, 1] = kpts[:, 1] - img_h / 2  # y
