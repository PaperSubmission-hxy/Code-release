...
def compute_tracks(num_images, num_keypoints_list, matches_list, track_degree,
    ...
    logger.info("----------ComputeTracks Begins----------")
    img_marked = np.zeros((num_images,), dtype=bool)
    touched = []
    ...
    for i, num_keypoints in tqdm(enumerate(num_keypoints_list), total=len(num_keypoints_list)):
    ...
    logger.info(f"{len(tracks)} tracks computed")
    logger.info("----------ComputeTracks Done----------")
