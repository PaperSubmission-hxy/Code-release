...
def pad_bottom_right(inp, pad_size, ret_mask=False):
    ...
    if pad_size >= 1500:
        logger.warning(f"pad_size is {pad_size}, >1500! cannot full pad due to oom")
        h_w = inp.shape[0] / inp.shape[1]
        if h_w >= 1:
            pad_size_h, pad_size_w = pad_size, round((int(pad_size / 1.3))//32)*32
        else:
            pad_size_h, pad_size_w = round((int(pad_size / 1.3))//32)*32, pad_size
    else:
...
def rotate_image(image, angle, rotate_with_extending=True):
    """
    image: np.array, h*w*c
    """
    if rotate_with_extending and angle % 90 == 0:
        assert angle >= 0
        # FIXED: assert image.shape[-1] == 1, "currently only support gray scale images, otherwise the following line will report error"

        if image.shape[-1] == 3:
            pil_img = PIL.Image.fromarray((image * 255).astype(np.uint8), mode="RGB")
        else:
            pil_img = PIL.Image.fromarray(np.uint8(image[:,:,0] * 255))

        out = pil_img.rotate(angle, expand=True)
        return (np.array(out) / 255).astype(np.float32), np.array([[angle]]) # 1*1
    else:
        image_center = tuple(np.array(image.shape[1::-1]) / 2)
        rot_mat = cv2.getRotationMatrix2D(image_center, angle, 1.0)
        img_rot = cv2.warpAffine(image, rot_mat, image.shape[1::-1], flags=cv2.INTER_LINEAR)
        transform_matrix = np.concatenate([rot_mat, np.array([[0,0,1]])], axis=0)
    return img_rot, transform_matrix


def read_rgb(path, mask=None, resize=None, resize_no_larger_than=False, resize_float=False, df=None, client=None,
...
def read_grayscale(path, mask=None, resize=None, resize_no_larger_than=False, resize_float=False, df=None, client=None,
                   pad_to=None, ret_scales=False, ret_pad_mask=False,
                   augmentor=None):
    resize = tuple(resize) if resize is not None else None
    if augmentor is None:
    ...
    image = resize_image(image, (w_new, h_new), interp="pil_LANCZOS").astype('float32')
    # image = resize_image(image, (w_new, h_new), interp="cv2_area").astype('float32')
