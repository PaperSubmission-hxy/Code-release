...
class MatchingMultiviewData(Dataset):
    ...
    def __init__(self, colmap_image_dataset, config, worker_split_idxs=None) -> None:
        ...
        t1 = time()
        logger.info(f"Assign bags takes: {t1 - t0} s")
        self.image_bags = self.chunk_bags()
        t2 = time()
        logger.info(f"Chunk bags takes: {t2 - t1} s")
    ...
    def __getitem__(self, index):
        ...
        data.update(
            ...
            {
                "query_points": reference_node.to(torch.float32),  # [M, 2] # M tracks
                "reference_points_coarse": query_nodes.transpose(
                    0, 1
                ).to(torch.float32),  # [N-1, M, 2]
